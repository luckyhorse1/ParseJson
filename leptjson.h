...
struct lept_value{
	union {
		...
		struct { lept_value* e; size_t size, capacity; }a; //使用数组（而不是链表）表示json的数组，这里的size代表数组的元素个数
...
void lept_set_array(lept_value* v, size_t capacity);// 初始化一个容量为capacity的数组
void lept_reserve_array(lept_value* v, size_t capacity); //容量不够时，扩充容量
void lept_shrink_array(lept_value* v); //缩小数组的容量至当前的有效元素大小
void lept_clear_array(lept_value* v);
lept_value* lept_pushback_array_element(lept_value* v); //往数组中插入一个元素，并返回该元素的指针
